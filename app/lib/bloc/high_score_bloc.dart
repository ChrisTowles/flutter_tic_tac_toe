...
class HighScoreBloc extends BlocBase {
  ...
  _handleFetchHighScores(_) {
    ...
    Firestore.instance.collection('scores').orderBy('wins', descending: true).limit(10).snapshots().listen((scoreSnapshot) async {
      if (scoreSnapshot.documents.isNotEmpty) {
        for (int i = 0; i < scoreSnapshot.documents.length; i++) {
          ...
          highScores.add(ScoreDetail(
              user: User(id: userDoc.documentID, name: userDetails['displayName']),
              losses: scoreDetails['losses'],
              wins: scoreDetails['wins'],
              wonLast: scoreDetails['wonLast']));
        }
